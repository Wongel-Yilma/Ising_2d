# This is the GPU+MPI version of the 2D Ising model.

How to run it on CHPC:

1. Load the appropraite modules: the gcc and openmpi shall support gpu
```bash
    module load gcc/11.2.0-gpu cuda/13.1 openmpi/4.1.4-gpu cmake/3.26.0
```
...
```bash
    mpirun -np $SLURM_NTASKS ./ising_2d_mpi_gpu in.input 
```